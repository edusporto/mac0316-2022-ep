...
import Tokenizer (isNumeral, isSymbol, isNumeralChar)
...
analyze tree = case tree of
  ...
  Pair first rest -> case first of
    ...
    where
      -- | Função auxiliar que analisa a posição `i` da árvore recursivamente.
      ...
      getSymbol i = case tree `index` i of
        Leaf symbol -> validId symbol
        _ -> error "ERRO analyze: símbolo esperado no lugar de uma expressão"
      validId :: String -> String
      validId id = if not (null id) && not (isNumeralChar (head id))
        then id
        else error "ERRO analyze: identificador inválido"
